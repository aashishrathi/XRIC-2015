...
test$mvar_2 <- as.numeric(sub(",","",sub("\\$","",test$mvar_2)))
test$mvar_3 <- as.numeric(sub(",","",sub("\\$","",test$mvar_3)))
test$mvar_4 <- as.numeric(sub(",","",sub("\\$","",test$mvar_4)))
test$mvar_5 <- as.numeric(sub(",","",sub("\\$","",test$mvar_5)))
test$mvar_6 <- as.numeric(sub(",","",sub("\\$","",test$mvar_6)))
#data <- data[sample(nrow(data)),]
#size <- nrow(data)
#data.train <- data[1:round(0.6*size),]
#data.test <- data[round((0.6*size) + 1):round(size),]

model <- gbm(actual_vote~. -Citizen.ID,data=data,distribution='multinomial',n.trees=200,interaction.depth=4,shrinkage=0.02,cv.folds = 5, bag.fraction = 0.7,verbose=TRUE)
pred <- predict(model,n.trees=200, newdata=test,type='response')
...
result <- cbind(as.character(test$Citizen.ID), p.pred_final)
write.table(result,"IBMWatson_IITBombay_1.csv", sep=",",row.names = FALSE, col.names = FALSE)
